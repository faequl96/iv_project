...
func TransactionRoutes(r *mux.Router, jwtServices jwtToken.JWTServices) {
	...
	r.HandleFunc("/transaction/reference-number/{referenceNumber}", middleware.Auth(jwtServices, h.GetTransactionByReferenceNumber)).Methods("GET")
	...
	r.HandleFunc("/transaction/id/{id}", middleware.Auth(jwtServices, h.DeleteTransaction)).Methods("DELETE")
}
